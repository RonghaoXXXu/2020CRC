...
with tf.compat.v1.Session() as sess:
    ...
    img = cv.imread('/home/ronghao/Desktop/test_images/2.jpg')
    rows = img.shape[0]
    cols = img.shape[1]
    ...
    for i in range(num_detections):
        ...
        if score > thresold:
            ...
            cv.putText(img, num_classes[classId-1]+':'+str(score), (int(x), int(y)),cv.FONT_HERSHEY_DUPLEX,0.3, (0,255,0), 1)         
    cv.imshow('TensorFlow MobileNet-V2-SSD', img)
    cv.waitKey(0)
